...
func NewSpring(deltaTimef, angularFrequencyf, dampingRatiof float32) (s Spring) {
	deltaTime := float64(deltaTimef)
	angularFrequency := float64(angularFrequencyf)
	dampingRatio := float64(dampingRatiof)
...
func (s Spring) Update(pos, vel float32, equilibriumPos float32) (newPos, newVel float32) {
	oldPos := float32(pos - equilibriumPos) // update in equilibrium relative space
	oldVel := float32(vel)

	newPos = (oldPos*float32(s.posPosCoef) + oldVel*float32(s.posVelCoef) + equilibriumPos)
	newVel = (oldPos*float32(s.velPosCoef) + oldVel*float32(s.velVelCoef))
